...
#---------------------------------------------------------------
def Print_Title():
    """ Function to print a title at the top of the ouput."""
...
def getLmer():
    """Retrieve the Lmer from an input of the user"""
    
    #constants for size, Lmer must be between 4 and 8
    MIN_SIZE = 4
    MAX_SIZE = 8
    ...
    ok = False
    while(not ok):
        Lmer = eval(input("enter an Lmer between 4 and 8: "))
        if(Lmer >= MIN_SIZE and Lmer <= MAX_SIZE):
            ok = True
    return Lmer        

def breakIntoMotifs(DNA, Lmer):
    """Move through the DNA sequence and store the Lmer's found in a list"""
    
    maxLmers = len(DNA) - Lmer + 1
    
    LmerList = []
    for nextLmer in range(0, maxLmers):
        LmerList.append( DNA[nextLmer:nextLmer+Lmer] )
        
    #for nextLmer in LmerList:
    #    print( nextLmer )    
